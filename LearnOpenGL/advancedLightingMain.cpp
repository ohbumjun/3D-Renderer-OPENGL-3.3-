...
void renderDirectionalLightDepthMap(Shader &directionalDepthMapShader,
                                    unsigned int directionalDepthMapFBO,
                                    int SHADOW_WIDTH,
                                    int SHADOW_HEIGHT,
                                    unsigned int floorTexture,
                                    glm::mat4 &lightSpaceMatrix);
void renderDirectionalShadow(Shader &directionalShadowMapShader,
                             int SHADOW_WIDTH,
                             int SHADOW_HEIGHT,
                             unsigned int floorTexture,
...
int main()
{
    ...
    Shader directionalDepthMapShader(vrxShaderPath.c_str(), fragShaderPath.c_str());
   ...
   std::string geoShaderPath = FileSystem::getPath(
    ...
    unsigned int directonalDepthMap;
    glGenTextures(1, &directonalDepthMap);
                                    ...
                                    glm::mat4 &lightSpaceMatrix)
{
    ...
    renderScene(directionalDepthMapShader);
     ...
     glm::mat4 &lightSpaceMatrix)
{
    ...
    glm::mat4 projection =
        glm::perspective(glm::radians(camera.Zoom),
                         ...
                         100.0f);
    glm::mat4 view = camera.GetViewMatrix();
    directionalShadowMapShader.setMat4("projection", projection);
    directionalShadowMapShader.setMat4("view", view);
    // set light uniforms
    directionalShadowMapShader.setVec3f("viewPos", camera.Position);
    directionalShadowMapShader.setVec3f("lightPos",
                                        depthMapDirectionalLightPos);
    directionalShadowMapShader.setMat4("lightSpaceMatrix", lightSpaceMatrix);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, floorTexture);
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, directonalDepthMap);
    ...
    glCullFace(GL_FRONT);
    renderScene(directionalShadowMapShader);
